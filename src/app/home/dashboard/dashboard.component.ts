...
export class DashboardComponent implements OnInit {
    ...
    public pieSeries: Object[] = [
        {
            dataSource: [
                { 'x': 'Principal Amount', y: this.data.principalValue * this.data.goldRate },
                { 'x': 'Interest Amount', y: ((this.data.emi * this.data.tent) ) }
            ],
            radius: '80%', xName: 'x',
    ...
    public principalAmt: string = (+this.data.principalAmt).toString();
    public principalValue : string = this.data.principalValue.toString();
